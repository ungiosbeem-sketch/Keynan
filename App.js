...
function LoginScreen({ navigation }) {
  ...
  const handleAuth = async () => {
    ...
    if (showSignup && !name) return Alert.alert('Error', 'Please enter your name');
  ...
  return (
    <LinearGradient colors={[COLORS.background, COLORS.card]} style={styles.loginContainer}>
      <Animated.View style={[styles.loginContent, { opacity: fadeAnim }]}>
        <Ionicons name="rocket" size={80} color={COLORS.pink} />
        <Text style={styles.loginTitle}>Ungio Premium</Text>
        <Text style={styles.loginSubtitle}>Plan Your Success</Text>
        
        {showSignup && (
          <TextInput placeholder="Full Name" placeholderTextColor={COLORS.gray} style={styles.loginInput} value={name} onChangeText={setName} />
        )}
        <TextInput placeholder="Email" placeholderTextColor={COLORS.gray} style={styles.loginInput} value={email} onChangeText={setEmail} autoCapitalize="none" />
        <TextInput placeholder="Password" placeholderTextColor={COLORS.gray} style={styles.loginInput} value={password} onChangeText={setPassword} secureTextEntry />
        
        <TouchableOpacity style={styles.loginButton} onPress={handleAuth}>
          <Text style={styles.loginButtonText}>{showSignup ? 'Sign Up' : 'Login'}</Text>
        </TouchableOpacity>
...
function HomeScreen({ tasks, toggleComplete, deleteTask, navigation }) {
  ...
  const renderTask = ({ item, index }) => (
    <Animated.View style={[styles.taskCard, { opacity: fadeAnim, transform: [{ translateX: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [50, 0] }) }] }]}>
      ...
      <View style={styles.taskContent}>
        ...
        <View style={styles.taskMeta}>
          <View style={styles.metaItem}><Feather name="calendar" size={12} color={COLORS.gray} /><Text style={styles.taskTime}> {item.date}</Text></View>
          <View style={styles.metaItem}><Feather name="clock" size={12} color={COLORS.gray} /><Text style={styles.taskTime}> {item.time}</Text></View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.welcomeHeader}>
          ...
          <View style={styles.premiumBadge}><FontAwesome5 name="crown" size={16} color={COLORS.gold} /><Text style={styles.premiumText}>PREMIUM</Text></View>
        </View>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput placeholder="Search tasks..." placeholderTextColor={COLORS.gray} style={styles.searchInput} value={searchQuery} onChangeText={setSearchQuery} />
        </View>

        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryScroll}>
          <TouchableOpacity style={[styles.categoryChip, selectedCategory === 'all' && styles.categoryChipActive]} onPress={() => setSelectedCategory('all')}><Text style={[styles.categoryChipText, selectedCategory === 'all' && styles.categoryChipTextActive]}>All</Text></TouchableOpacity>
          {categories.map(cat => <TouchableOpacity key={cat.id} style={[styles.categoryChip, selectedCategory === cat.id && styles.categoryChipActive]} onPress={() => setSelectedCategory(cat.id)}><Feather name={cat.icon} size={14} color={selectedCategory === cat.id ? COLORS.pink : COLORS.gray} /><Text style={[styles.categoryChipText, selectedCategory === cat.id && styles.categoryChipTextActive]}>{cat.title}</Text></TouchableOpacity>)}
        </ScrollView>

        <FlatList data={filteredTasks} keyExtractor={item => item.id} renderItem={renderTask} scrollEnabled={false} ListEmptyComponent={<View style={styles.emptyContainer}><Ionicons name="clipboard-outline" size={80} color={COLORS.gray} /><Text style={styles.emptyText}>No tasks yet</Text><Text style={styles.emptySubtext}>Tap + to add your first task</Text></View>} />
      </ScrollView>
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AddTask')}><Feather name="plus" size={28} color="#000" /></TouchableOpacity>
    </SafeAreaView>
  );
}
...
function AddTaskScreen({ navigation, tasks, setTasks }) {
  ...
  const saveTask = () => {
    if (!title.trim()) return Alert.alert('Error', 'Please enter task title');
    const newTask = { id: Date.now().toString(), title, description, date: date.toLocaleDateString(), time: time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), reminder, completed: false, priority, category: selectedCategory };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.modalHeader}><TouchableOpacity onPress={() => navigation.goBack()}><Feather name="arrow-left" size={28} color={COLORS.white} /></TouchableOpacity><Text style={styles.modalTitle}>New Task</Text><View style={{ width: 28 }} /></View>
      <ScrollView showsVerticalScrollIndicator={false}>
        <TextInput placeholder="Task title" placeholderTextColor={COLORS.gray} style={styles.input} value={title} onChangeText={setTitle} />
        <TextInput placeholder="Description (optional)" placeholderTextColor={COLORS.gray} style={[styles.input, { height: 100 }]} multiline value={description} onChangeText={setDescription} />
        
        <Text style={styles.inputLabel}>Category</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categorySelect}>
          {categories.map(cat => <TouchableOpacity key={cat.id} style={[styles.categorySelectItem, selectedCategory === cat.id && { borderColor: cat.color, backgroundColor: cat.color + '20' }]} onPress={() => setSelectedCategory(cat.id)}><Feather name={cat.icon} size={20} color={selectedCategory === cat.id ? cat.color : COLORS.gray} /><Text style={[styles.categorySelectText, selectedCategory === cat.id && { color: cat.color }]}>{cat.title}</Text></TouchableOpacity>)}
        </ScrollView>

        <Text style={styles.inputLabel}>Priority</Text>
        <View style={styles.priorityRow}>
          {['low', 'medium', 'high'].map(p => <TouchableOpacity key={p} style={[styles.priorityBtn, priority === p && { backgroundColor: p === 'high' ? COLORS.danger : p === 'medium' ? COLORS.warning : COLORS.success }]} onPress={() => setPriority(p)}><Text style={styles.priorityBtnText}>{p.toUpperCase()}</Text></TouchableOpacity>)}
        </View>

        <TouchableOpacity style={styles.dateBtn} onPress={() => setShowDate(true)}><Feather name="calendar" size={20} color={COLORS.white} /><Text style={styles.dateBtnText}>{date.toLocaleDateString()}</Text></TouchableOpacity>
        <TouchableOpacity style={styles.dateBtn} onPress={() => setShowTime(true)}><Feather name="clock" size={20} color={COLORS.white} /><Text style={styles.dateBtnText}>{time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text></TouchableOpacity>
        ...
        <View style={styles.reminderBox}><Text style={styles.reminderText}>🔔 Set Reminder</Text><Switch value={reminder} onValueChange={setReminder} trackColor={{ true: COLORS.pink }} /></View>
        <TouchableOpacity style={styles.saveBtn} onPress={saveTask}><LinearGradient colors={[COLORS.pink, COLORS.pink2]} style={styles.gradientBtn}><Text style={styles.saveBtnText}>✨ Create Task</Text></LinearGradient></TouchableOpacity>
      </ScrollView>
    </SafeAreaView>
  );
}
...
function StatsScreen({ tasks }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <LinearGradient colors={[COLORS.pink + '40', COLORS.purple + '40']} style={styles.mainStatCard}>
          <Text style={styles.mainStatNumber}>{completionRate}%</Text>
          <Text style={styles.mainStatLabel}>Completion Rate</Text>
          <View style={styles.mainStatRing}><Text style={styles.mainStatRingText}>{completed}/{tasks.length}</Text></View>
        </LinearGradient>

        <View style={styles.statsRow}>
          <View style={styles.statMiniCard}><Text style={styles.statMiniNumber}>{tasks.length}</Text><Text style={styles.statMiniLabel}>Total</Text></View>
          <View style={styles.statMiniCard}><Text style={styles.statMiniNumber}>{completed}</Text><Text style={styles.statMiniLabel}>Done</Text></View>
          <View style={styles.statMiniCard}><Text style={styles.statMiniNumber}>{pending}</Text><Text style={styles.statMiniLabel}>Pending</Text></View>
        </View>

        <Text style={styles.sectionSubtitle}>By Category</Text>
        {byCategory.map(cat => cat.count > 0 && <View key={cat.id} style={styles.categoryStat}><View style={styles.categoryStatLeft}><Feather name={cat.icon} size={18} color={cat.color} /><Text style={styles.categoryStatName}>{cat.title}</Text></View><Text style={styles.categoryStatCount}>{cat.count}</Text></View>)}
        
        <View style={styles.quoteCard}><Text style={styles.quoteText}>"The secret of getting ahead is getting started."</Text><Text style={styles.quoteAuthor}>- Mark Twain</Text></View>
      </ScrollView>
    </SafeAreaView>
  );
}
...
function ProfileScreen({ navigation }) {
  ...
  const handleLogout = () => { Alert.alert('Logout', 'Are you sure?', [{ text: 'Cancel' }, { text: 'Logout', onPress: () => { logout(); navigation.replace('Login'); } }]); };

  return (
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <LinearGradient colors={[COLORS.pink, COLORS.purple]} style={styles.profileHeader}>
          <View style={styles.profileAvatar}><Text style={styles.profileAvatarText}>{user?.name?.[0] || 'U'}</Text></View>
          <Text style={styles.profileName}>{user?.name}</Text>
          <Text style={styles.profileEmail}>{user?.email}</Text>
          <View style={styles.premiumCard}><FontAwesome5 name="crown" size={20} color={COLORS.gold} /><Text style={styles.premiumCardText}>Premium Member</Text></View>
        </LinearGradient>

        <View style={styles.settingsSection}>
          <Text style={styles.settingsTitle}>Preferences</Text>
          <View style={styles.settingItem}><View style={styles.settingLeft}><Feather name="moon" size={20} color={COLORS.pink} /><Text style={styles.settingText}>Dark Mode</Text></View><Switch value={darkMode} onValueChange={setDarkMode} /></View>
          <View style={styles.settingItem}><View style={styles.settingLeft}><Feather name="bell" size={20} color={COLORS.pink} /><Text style={styles.settingText}>Notifications</Text></View><Switch value={notifications} onValueChange={setNotifications} /></View>
        </View>

        <View style={styles.settingsSection}>
          <Text style={styles.settingsTitle}>Account</Text>
          <TouchableOpacity style={styles.settingItem}><View style={styles.settingLeft}><Feather name="edit-2" size={20} color={COLORS.pink} /><Text style={styles.settingText}>Edit Profile</Text></View><Feather name="chevron-right" size={20} color={COLORS.gray} /></TouchableOpacity>
          <TouchableOpacity style={styles.settingItem}><View style={styles.settingLeft}><Feather name="lock" size={20} color={COLORS.pink} /><Text style={styles.settingText}>Change Password</Text></View><Feather name="chevron-right" size={20} color={COLORS.gray} /></TouchableOpacity>
        </View>

        <TouchableOpacity style={styles.logoutBtn} onPress={handleLogout}><Feather name="log-out" size={20} color={COLORS.danger} /><Text style={styles.logoutText}>Logout</Text></TouchableOpacity>
...
function MainApp() {
  ...
  const loadTasks = async () => { const data = await AsyncStorage.getItem(STORAGE_KEY); if (data) setTasks(JSON.parse(data)); };
  const saveTasks = async () => { await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(tasks)); };
  const toggleComplete = (id) => { setTasks(tasks.map(task => task.id === id ? { ...task, completed: !task.completed } : task)); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); };
  const deleteTask = (id) => { Alert.alert('Delete', 'Remove this task?', [{ text: 'Cancel' }, { text: 'Delete', onPress: () => setTasks(tasks.filter(task => task.id !== id)) }]); };

  return (
    <Tab.Navigator screenOptions={{ headerShown: false, tabBarStyle: styles.tabBar, tabBarShowLabel: false, tabBarActiveTintColor: COLORS.pink, tabBarInactiveTintColor: COLORS.gray, tabBarIcon: ({ color, size, route }) => { let icon; if (route.name === 'Home') icon = 'home'; if (route.name === 'Stats') icon = 'bar-chart-2'; if (route.name === 'Profile') icon = 'user'; return <Feather name={icon} size={size} color={color} />; } }}>
      <Tab.Screen name="Home">{props => <HomeScreen {...props} tasks={tasks} toggleComplete={toggleComplete} deleteTask={deleteTask} />}</Tab.Screen>
      <Tab.Screen name="Stats">{props => <StatsScreen {...props} tasks={tasks} />}</Tab.Screen>
      <Tab.Screen name="Profile">{props => <ProfileScreen {...props} navigation={props.navigation} />}</Tab.Screen>
      <Tab.Screen name="AddTask" options={{ tabBarButton: () => null }}>{props => <AddTaskScreen {...props} tasks={tasks} setTasks={setTasks} />}</Tab.Screen>
    </Tab.Navigator>
  );
}
...
export default function App() {
  ...
  if (!isReady) return <View style={styles.splashContainer}><ActivityIndicator size="large" color={COLORS.pink} /><Text style={styles.splashText}>Ungio Premium</Text></View>;

  return (
    <SafeAreaProvider>
      <AuthProvider>
        <NavigationContainer>
          <AuthContext.Consumer>{({ user, loading }) => loading ? <View style={styles.splashContainer}><ActivityIndicator size="large" color={COLORS.pink} /></View> : user ? <MainApp /> : <LoginScreen navigation={{ replace: () => {} }} />}</AuthContext.Consumer>
        </NavigationContainer>
      </AuthProvider>
    </SafeAreaProvider>
  );
...
const styles = StyleSheet.create({
  ...
  loginTitle: { fontSize: 32, fontWeight: 'bold', color: COLORS.white, marginTop: 20 },
  ...
  mainStatNumber: { fontSize: 48, fontWeight: 'bold', color: COLORS.white },
  mainStatLabel: { color: COLORS.white, opacity: 0.8, marginTop: 5 },
  ...
  profileAvatar: { width: 80, height: 80, borderRadius: 40, backgroundColor: COLORS.white, justifyContent: 'center', alignItems: 'center' },
  profileAvatarText: { fontSize: 36, fontWeight: 'bold', color: COLORS.pink },
  profileName: { fontSize: 24, fontWeight: 'bold', color: COLORS.white, marginTop: 12 },
  profileEmail: { color: COLORS.white, opacity: 0.7, marginTop: 4 },
