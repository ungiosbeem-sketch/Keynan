...
function ProfileScreen({ navigation }) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission needed', 'Please grant gallery permission to upload photo');
        return;
      }
      ...
      if (!result.canceled) {
        ...
        const success = await updateUserAvatar(urlData.publicUrl);
  ...
  const takePhoto = async () => {
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('Permission needed', 'Please grant camera permission to take photo');
        return;
      }
