...
import {
  ...
  Platform, Image, RefreshControl, Vibration, Modal,
} from 'react-native';
...
const quotes = [
  ...
  { text: "Read 500 pages like this every day.", author: "Warren Buffett" },
...
const categories = [
  ...
  { id: 'school', title: 'School', icon: 'book', color: '#FF9F0A' },
];
...
function SchoolProvider({ children }) {
  ...
  const addCustomSubject = async (subject) => {
    ...
    Alert.alert('Success', 'Subject added to schedule!');
  };

  const deleteCustomSubject = async (id) => {
    Alert.alert('Delete', 'Remove this subject from schedule?', [
      { text: 'Cancel' },
...
function ReadingProvider({ children }) {
  ...
  const addPdfBook = async (pdfUri, fileName, fileSize) => {
    try {
      ...
      Alert.alert('Success', 'PDF added to your reading list!');
      return true;
    } catch (error) {
  ...
  return (
    <ReadingContext.Provider value={{ 
      readingList, pdfBooks, currentBook, addBook, addPdfBook, updateProgress, 
  ...
  return (
    <LinearGradient colors={darkMode ? ['#000000', '#1A1A2E'] : ['#F5F5F5', '#E8E8E8']} style={styles.loginContainer}>
      <Animated.View style={[styles.loginContent, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
        ...
        {showSignup && (
          <TextInput
            placeholder="Full Name" placeholderTextColor={COLORS.gray}
        ...
        />
        <TextInput
          placeholder="Password" placeholderTextColor={COLORS.gray}
          style={[styles.loginInput, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}
        ...
        {!showSignup && (
          <TouchableOpacity onPress={() => Alert.alert('Reset Password', 'Check your email')}>
            <Text style={[styles.forgotPassword, { color: COLORS.primary }]}>Forgot Password?</Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setShowSignup(!showSignup); }}>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.welcomeHeader}>
          <View>
            <Text style={[styles.greetingText, { color: COLORS.gray }]}>{getGreeting()},</Text>
        ...
        <View style={styles.quickStatsRow}>
          <View style={[styles.quickStat, { backgroundColor: COLORS.routine + '15' }]}>
            <Feather name="sunrise" size={20} color={COLORS.routine} />
            <Text style={[styles.quickStatNumber, { color: COLORS.routine }]}>{routineDone}/10</Text>
            <Text style={styles.quickStatLabel}>Routine</Text>
          </View>
          <View style={[styles.quickStat, { backgroundColor: COLORS.school + '15' }]}>
            <Feather name="book" size={20} color={COLORS.school} />
            <Text style={[styles.quickStatNumber, { color: COLORS.school }]}>{pendingHomework}</Text>
            <Text style={styles.quickStatLabel}>Homework</Text>
          </View>
          <View style={[styles.quickStat, { backgroundColor: COLORS.reading + '15' }]}>
            <Feather name="book-open" size={20} color={COLORS.reading} />
            <Text style={[styles.quickStatNumber, { color: COLORS.reading }]}>{activeBooks + pdfBooks.length}</Text>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.routineHeader}>
          ...
          <Text style={[styles.routineSubtitle, { color: COLORS.gray }]}>Good {getGreeting()}! Stay on track</Text>
        </View>
        ...
        {routine.map((item, index) => (
          <Animated.View key={item.id} style={[styles.routineItem, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
...
// ============ SCHOOL SCREEN (With Editable Schedule) ============
function SchoolScreen() {
  const { schedule, homework, addHomework, toggleHomework, deleteHomework, 
    addCustomSubject, deleteCustomSubject, showAddSubject, setShowAddSubject, 
    newSubject, setNewSubject, weekDays } = useContext(SchoolContext);
  const { darkMode } = useContext(ThemeContext);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={[styles.editScheduleBtn, { backgroundColor: COLORS.school + '20' }]} onPress={() => setShowAddSubject(true)}>
          <Feather name="edit-2" size={16} color={COLORS.school} />
          <Text style={[styles.editScheduleText, { color: COLORS.school }]}>Edit Schedule</Text>
        </TouchableOpacity>
          ...
          {filteredSchedule.length > 0 ? filteredSchedule.map(cls => (
            <View key={cls.id} style={styles.scheduleItem}>
              <View style={styles.scheduleTimeBox}>
                <Text style={[styles.scheduleTime, { color: COLORS.school }]}>{cls.time}</Text>
              </View>
              ...
              {cls.isCustom && (
                <TouchableOpacity onPress={() => deleteCustomSubject(cls.id)}>
                  <Feather name="trash-2" size={16} color={COLORS.danger} />
                </TouchableOpacity>
              )}
              {!cls.isCustom && <Feather name="bookmark" size={20} color={COLORS.school} />}
            </View>
          )) : <Text style={[styles.emptyText, { color: COLORS.gray }]}>No classes scheduled. Tap Edit Schedule to add!</Text>}
        </View>
        ...
        <View style={[styles.homeworkSection, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
          <View style={styles.homeworkHeader}>
            ...
            <TouchableOpacity style={styles.addHomeworkBtn} onPress={() => setShowAddHomework(true)}>
          ...
          {todayHomework.map(hw => (
            <View key={hw.id} style={styles.homeworkItem}>
              <TouchableOpacity onPress={() => toggleHomework(hw.id)}>
                <Feather name={hw.completed ? 'check-circle' : 'circle'} size={20} color={hw.completed ? COLORS.success : COLORS.school} />
              </TouchableOpacity>
              ...
              <TouchableOpacity onPress={() => deleteHomework(hw.id)}>
                <Feather name="trash-2" size={16} color={COLORS.gray} />
              </TouchableOpacity>
            </View>
          ))}
          {todayHomework.length === 0 && <Text style={[styles.emptyText, { color: COLORS.gray }]}>No homework pending 🎉</Text>}
        </View>

        {completedHomework.length > 0 && (
          <View style={[styles.completedSection, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.card }]}>
            <Text style={[styles.sectionTitle, { color: COLORS.success }]}>✅ Completed</Text>
            {completedHomework.map(hw => (
              <View key={hw.id} style={styles.completedHomeworkItem}>
                <Feather name="check-circle" size={16} color={COLORS.success} />
                <Text style={[styles.completedHomeworkText, { color: COLORS.gray }]}>{hw.subject} - {hw.description}</Text>
              </View>
            ))}
          </View>
        )}
      ...
      <Modal visible={showAddHomework} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
            ...
            <TextInput placeholder="Subject" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={homeworkSubject} onChangeText={setHomeworkSubject} />
            <TextInput placeholder="Description" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={homeworkDesc} onChangeText={setHomeworkDesc} multiline />
            <TouchableOpacity style={[styles.modalButton, { backgroundColor: COLORS.school }]} onPress={handleAddHomework}>
              <Text style={styles.modalButtonText}>Add Homework</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.modalCancel, { borderColor: COLORS.gray }]} onPress={() => setShowAddHomework(false)}>
      ...
      <Modal visible={showAddSubject} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
            <Text style={[styles.modalTitle, { color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}>Add Custom Subject</Text>
            
            <View style={styles.modalRow}>
              <Text style={[styles.modalLabel, { color: COLORS.gray }]}>Day:</Text>
              <View style={styles.modalDaySelector}>
                {weekDays.map(day => (
                  <TouchableOpacity key={day} style={[styles.modalDayChip, newSubject.day === day && { backgroundColor: COLORS.school }]} onPress={() => setNewSubject({...newSubject, day})}>
                    <Text style={[styles.modalDayText, newSubject.day === day && { color: '#000' }]}>{day.slice(0, 3)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
            
            <TextInput placeholder="Subject Name" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={newSubject.subject} onChangeText={(text) => setNewSubject({...newSubject, subject: text})} />
            <TextInput placeholder="Time (e.g., 08:00-09:30)" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={newSubject.time} onChangeText={(text) => setNewSubject({...newSubject, time: text})} />
            <TextInput placeholder="Teacher (optional)" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={newSubject.teacher} onChangeText={(text) => setNewSubject({...newSubject, teacher: text})} />
            <TextInput placeholder="Room (optional)" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={newSubject.room} onChangeText={(text) => setNewSubject({...newSubject, room: text})} />
            
            <TouchableOpacity style={[styles.modalButton, { backgroundColor: COLORS.school }]} onPress={() => {
              if (newSubject.subject && newSubject.time) {
                addCustomSubject(newSubject);
                setShowAddSubject(false);
                setNewSubject({ day: 'Monday', subject: '', time: '', teacher: '', room: '' });
              } else {
                Alert.alert('Error', 'Please fill subject and time');
              }
            }}>
              <Text style={styles.modalButtonText}>Add to Schedule</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.modalCancel, { borderColor: COLORS.gray }]} onPress={() => setShowAddSubject(false)}>
              <Text style={[styles.modalCancelText, { color: COLORS.gray }]}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
...
// ============ READING SCREEN (With PDF Upload) ============
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.readingStatsCard, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
          <View style={styles.readingStatsRow}>
            <View style={styles.readingStatItem}>
              <Text style={[styles.readingStatNumber, { color: COLORS.reading }]}>{readingList.length + pdfBooks.length}</Text>
              <Text style={[styles.readingStatLabel, { color: COLORS.gray }]}>Books</Text>
            </View>
            <View style={styles.readingStatItem}>
              <Text style={[styles.readingStatNumber, { color: COLORS.reading }]}>{totalRead}</Text>
              <Text style={[styles.readingStatLabel, { color: COLORS.gray }]}>Pages Read</Text>
            </View>
            <View style={styles.readingStatItem}>
              <Text style={[styles.readingStatNumber, { color: COLORS.reading }]}>{readingProgress}%</Text>
              <Text style={[styles.readingStatLabel, { color: COLORS.gray }]}>Progress</Text>
            </View>
          </View>
          <View style={styles.readingProgressBarContainer}>
            <View style={[styles.readingProgressBar, { width: `${readingProgress}%`, backgroundColor: COLORS.reading }]} />
          </View>
        </View>

        <TouchableOpacity style={[styles.addBookBtn, { backgroundColor: COLORS.reading }]} onPress={() => setShowAddBook(true)}>
          <Feather name="plus" size={20} color="#000" />
          <Text style={styles.addBookBtnText}>Add New Book</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.addPdfBtn, { backgroundColor: COLORS.reading + '80' }]} onPress={pickPdf}>
          <Feather name="file-text" size={20} color="#000" />
          <Text style={styles.addPdfBtnText}>Upload PDF Book</Text>
        </TouchableOpacity>
            ...
            <View key={book.id} style={[styles.bookCard, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
              <View style={styles.bookHeader}>
                <View style={styles.bookIcon}>
                  <FontAwesome5 name="book" size={24} color={COLORS.reading} />
                </View>
                <View style={styles.bookInfo}>
                  <Text style={[styles.bookTitle, { color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}>{book.title}</Text>
                  <Text style={[styles.bookAuthor, { color: COLORS.gray }]}>by {book.author}</Text>
                </View>
              ...
              <View style={styles.bookProgressContainer}>
                <View style={styles.bookProgressBarBg}>
                  <View style={[styles.bookProgressBar, { width: `${bookProgress}%`, backgroundColor: COLORS.reading }]} />
                </View>
              ...
              {book.completed && <View style={styles.completedBadge}><Text style={styles.completedBadgeText}>🎉 Completed!</Text></View>}
            </View>
          );
        })}
        ...
        {pdfBooks.map(book => (
          <View key={book.id} style={[styles.bookCard, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
            <View style={styles.bookHeader}>
              <View style={[styles.bookIcon, { backgroundColor: COLORS.reading + '20' }]}>
                <Feather name="file-text" size={24} color={COLORS.reading} />
              </View>
              <View style={styles.bookInfo}>
                <Text style={[styles.bookTitle, { color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}>{book.title}</Text>
                <Text style={[styles.bookAuthor, { color: COLORS.gray }]}>PDF Document</Text>
              </View>
              <TouchableOpacity onPress={() => deletePdfBook(book.id)}>
                <Feather name="trash-2" size={18} color={COLORS.gray} />
              </TouchableOpacity>
            </View>
      ...
      <Modal visible={showAddBook} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
            ...
            <TextInput placeholder="Book Title" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={bookTitle} onChangeText={setBookTitle} />
            <TextInput placeholder="Author" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={bookAuthor} onChangeText={setBookAuthor} />
            <TextInput placeholder="Total Pages" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={bookPages} onChangeText={setBookPages} keyboardType="numeric" />
            <TouchableOpacity style={[styles.modalButton, { backgroundColor: COLORS.reading }]} onPress={handleAddBook}>
              <Text style={styles.modalButtonText}>Add Book</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.modalCancel, { borderColor: COLORS.gray }]} onPress={() => setShowAddBook(false)}>
              <Text style={[styles.modalCancelText, { color: COLORS.gray }]}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <Modal visible={showProgressModal} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
            ...
            <TextInput placeholder="Pages read" placeholderTextColor={COLORS.gray} style={[styles.modalInput, { backgroundColor: darkMode ? COLORS.cardLight : LIGHT_COLORS.border, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]} value={pagesRead} onChangeText={setPagesRead} keyboardType="numeric" />
            <TouchableOpacity style={[styles.modalButton, { backgroundColor: COLORS.reading }]} onPress={handleUpdateProgress}>
              <Text style={styles.modalButtonText}>Update</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.modalCancel, { borderColor: COLORS.gray }]} onPress={() => { setShowProgressModal(false); setPagesRead(''); }}>
              <Text style={[styles.modalCancelText, { color: COLORS.gray }]}>Cancel</Text>
            </TouchableOpacity>
          </View>
        </View>
...
// ============ PROFILE SCREEN (Fixed Avatar Upload) ============
function ProfileScreen({ navigation }) {
  ...
  const uploadImage = async (uri, mimeType) => {
    ...
    const { data, error } = await supabase.storage.from('avatars').upload(fileName, blob, { contentType: mimeType, upsert: true });
    if (error) throw error;
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled) {
        ...
        const avatarUrl = await uploadImage(image.uri, image.mimeType);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView>
        ...
        <View style={[styles.settingsSection, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
          <Text style={[styles.settingsTitle, { color: COLORS.gray }]}>Support</Text>
          <TouchableOpacity style={[styles.settingItem, { borderBottomColor: darkMode ? COLORS.dark : LIGHT_COLORS.border }]} onPress={() => Alert.alert('About', 'Keynan v2.0.0\nStudy • Routine • Read\n\nMade with ❤️')}>
            <View style={styles.settingLeft}><Feather name="info" size={20} color={COLORS.primary} /><Text style={[styles.settingText, { color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}>About</Text></View>
...
const styles = StyleSheet.create({
  ...
  routineHeader: { marginBottom: 20 },
  routineSubtitle: { fontSize: 14, marginTop: 4 },
  ...
  completedSection: { borderRadius: 20, padding: 16, marginBottom: 20 },
  completedHomeworkItem: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 6 },
  completedHomeworkText: { fontSize: 12 },
  editScheduleBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, padding: 10, borderRadius: 20, marginBottom: 16, alignSelf: 'flex-start' },
  editScheduleText: { fontSize: 12, fontWeight: '500' },
  ...
  completedBadge: { backgroundColor: COLORS.success + '20', padding: 6, borderRadius: 8, alignItems: 'center' },
  completedBadgeText: { fontSize: 12, color: COLORS.success },
  ...
  modalDaySelector: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
  modalDayChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, backgroundColor: COLORS.cardLight },
  modalDayText: { fontSize: 12, color: COLORS.gray },
});

// No duplicate LinearGradient declaration needed - already imported at top
