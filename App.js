...
import {
  ...
  Platform, Image, RefreshControl, Modal,
} from 'react-native';
...
function SchoolProvider({ children }) {
  ...
  const deleteCustomSubject = async (id) => {
    Alert.alert('Delete', 'Remove this subject?', [
      { text: 'Cancel' },
  ...
  return (
    <View style={[styles.loginContainer, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      <Animated.View style={[styles.loginContent, { opacity: fadeAnim }]}>
        ...
        {showSignup && (
          <TextInput placeholder="Full Name" placeholderTextColor={COLORS.gray}
        ...
        <TextInput placeholder="Password" placeholderTextColor={COLORS.gray}
          style={[styles.loginInput, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card, color: darkMode ? COLORS.white : LIGHT_COLORS.text }]}
...
// ============ ADD TASK SCREEN (FIXED - No RLS Error) ============
...
function SchoolScreen() {
  const { schedule, homework, addHomework, toggleHomework, deleteHomework, showAddSubject, setShowAddSubject, newSubject, setNewSubject } = useContext(SchoolContext);
  const { darkMode } = useContext(ThemeContext);
...
function ProfileScreen({ navigation }) {
  ...
  const uploadImage = async (uri, mimeType) => {
    ...
    const { error } = await supabase.storage.from('avatars').upload(fileName, blob, { contentType: mimeType || 'image/jpeg', upsert: true });
    if (error) throw error;
