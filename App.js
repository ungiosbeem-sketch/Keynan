...
function SchoolProvider({ children }) {
  ...
  const addScheduleItem = (item) => {
    ...
    AsyncStorage.setItem('@school_schedule', JSON.stringify([...schedule, item]));
  };

  return (
    <SchoolContext.Provider value={{ schedule, homework, addHomework, toggleHomework, deleteHomework, addScheduleItem }}>
      {children}
...
function ReadingProvider({ children }) {
  ...
  const addBook = (title, author, totalPages) => {
    const newBook = { id: Date.now().toString(), title, author, totalPages: parseInt(totalPages), currentPage: 0, completed: false };
  ...
  const setCurrentBookFn = (book) => {
    setCurrentBook(book);
  };

  return (
    <ReadingContext.Provider value={{ readingList, currentBook, addBook, updateProgress, deleteBook, setCurrentBook: setCurrentBookFn }}>
      {children}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {routine.map((item, index) => (
          <Animated.View key={item.id} style={[styles.routineItem, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card, opacity: 1, transform: [{ translateX: 0 }] }]}>
          ...
          {filteredSchedule.length > 0 ? filteredSchedule.map(cls => (
            <View key={cls.id} style={styles.scheduleItem}>
              ...
              <View style={styles.scheduleInfo}>
                ...
                <Text style={[styles.scheduleDetail, { color: COLORS.gray }]}>{cls.teacher} • Room {cls.room}</Text>
              </View>
              <Feather name="bookmark" size={20} color={COLORS.school} />
            </View>
          )) : <Text style={[styles.emptyText, { color: COLORS.gray }]}>No classes scheduled</Text>}
        </View>
...
function ReadingScreen() {
  const { readingList, addBook, updateProgress, deleteBook } = useContext(ReadingContext);
  const { darkMode } = useContext(ThemeContext);
        ...
        <View style={[styles.readingStatsCard, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
          <View style={styles.readingStatsRow}>
            <View style={styles.readingStatItem}>
              <Text style={[styles.readingStatNumber, { color: COLORS.reading }]}>{readingList.length}</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.quickStatsRow}>
          ...
          <View style={[styles.quickStat, { backgroundColor: COLORS.reading + '15' }]}>
            ...
            <Text style={[styles.quickStatNumber, { color: COLORS.reading }]}>{activeBooks}</Text>
...
function ProfileScreen({ navigation }) {
  ...
  const uploadImage = async (uri, mimeType) => {
    ...
    const { error } = await supabase.storage.from('avatars').upload(fileName, blob, { contentType: mimeType, upsert: true });
    if (error) throw error;
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setUploading(true);
      const avatarUrl = await uploadImage(result.assets[0].uri, result.assets[0].mimeType);
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: darkMode ? COLORS.background : LIGHT_COLORS.background }]}>
      ...
      <ScrollView>
        <View style={[styles.profileHeader, { backgroundColor: darkMode ? COLORS.card : LIGHT_COLORS.card }]}>
          <TouchableOpacity onPress={() => Alert.alert('Profile Photo', 'Choose option', [{ text: 'Choose from Gallery', onPress: pickImage }, { text: 'Cancel' }])} disabled={uploading}>
...
const styles = StyleSheet.create({
  ...
  quickStatLabel: { fontSize: 10, color: COLORS.gray, marginTop: 2 },
  ...
  addBookBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 14, borderRadius: 12, marginBottom: 20 },
