  ...
  return (
    <View style={styles.loginContainer}>
      <Animated.View style={[styles.loginContent, { opacity: fadeAnim }]}>
        ...
        <Text style={styles.loginSubtitle}>Modern Productivity Planner</Text>
...
function HomeScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AddTask', { onTaskAdded: loadTasks })}>
  ...
  const getMarkedDates = () => {
    ...
    tasks.forEach(task => {
      if (task.date) {
        ...
        marked[formattedDate] = { marked: true, dotColor: COLORS.pink };
      }
    });
    marked[selectedDate] = { selected: true, selectedColor: COLORS.pink };
    return marked;
  };

  const tasksForSelectedDate = tasks.filter(task => task.date === selectedDate.split('-').reverse().join('/'));
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.calendarTasksList}>
        ...
        {tasksForSelectedDate.map(task => (
          <View key={task.id} style={styles.calendarTaskItem}>
            <Feather name={task.completed ? 'check-circle' : 'circle'} size={20} color={COLORS.pink} />
            <Text style={[styles.calendarTaskText, task.completed && styles.completedTask]}>{task.title}</Text>
          </View>
        ))}
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.goalsHeader}>
          ...
          <View style={styles.progressContainer}>
            ...
            <Text style={styles.progressStats}>{completedCount}/{goals.length} completed • {progress}%</Text>
          </View>
        </View>
        ...
        {goals.map(goal => (
          <View key={goal.id} style={styles.goalItem}>
            <TouchableOpacity onPress={() => toggleGoal(goal.id)} style={styles.checkbox}>
              <Feather name={goal.completed ? 'check-circle' : 'circle'} size={22} color={COLORS.pink} />
            </TouchableOpacity>
            <Text style={[styles.goalText, goal.completed && styles.completedTask]}>{goal.text}</Text>
      ...
      screenOptions={{
        ...
        tabBarIcon: ({ color, size, focused }) => {
          return {
            ...
            Calendar: <Feather name="calendar" size={size} color={color} />,
            Goals: <Feather name="flag" size={size} color={color} />,
            Stats: <Feather name="bar-chart-2" size={size} color={color} />,
...
const styles = StyleSheet.create({
  ...
  splashText: { color: COLORS.pink, fontSize: 32, fontWeight: 'bold', marginTop: 20 },
  ...
  loginSubtitle: { color: COLORS.gray, marginBottom: 30, textAlign: 'center' },
  ...
  forgotPassword: { color: COLORS.pink, marginTop: 15, textAlign: 'center', fontSize: 14 },
  ...
  calendarTaskItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.card, padding: 14, borderRadius: 12, marginBottom: 10, gap: 12 },
  calendarTaskText: { color: COLORS.white, flex: 1, fontSize: 14 },
  ...
  goalInput: { flex: 1, backgroundColor: COLORS.card, borderRadius: 12, padding: 14, color: COLORS.white },
  addGoalButton: { backgroundColor: COLORS.pink, width: 50, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  goalItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.card, padding: 15, borderRadius: 12, marginBottom: 10, gap: 12 },
  goalText: { flex: 1, color: COLORS.white, fontSize: 16 },
